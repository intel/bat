		...
		float mean, float p, int channel, int start)
{
	...
	if (hz_peak < DC_THRESHOLD) {
		fprintf(bat->log, _(" WARNING: Found low peak %2.2f Hz,"),
				hz_peak);
		fprintf(bat->log, _(" very close to DC\n"));
		ret = FOUND_DC;
	} else if (hz_peak < bat->target_freq[channel] - tolerance) {
		fprintf(bat->log, _(" FAIL: Peak freq too low %2.2f Hz\n"),
				hz_peak);
		ret = FOUND_WRONG_PEAK;
	} else if (hz_peak > bat->target_freq[channel] + tolerance) {
		fprintf(bat->log, _(" FAIL: Peak freq too high %2.2f Hz\n"),
				hz_peak);
		ret = FOUND_WRONG_PEAK;
	} else {
...
int analyze_capture(struct bat *bat)
{
	...
	fprintf(bat->log, _("\nBAT analyses signal has %d frames at %d Hz,"),
			bat->frames, bat->rate);
